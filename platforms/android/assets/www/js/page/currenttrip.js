	...
	$(function()
	{
		...
		if (OTPdata)
		{
			...
			for (var i = 0; i < legs.length; ++i)
			{
				...
				trips.add()
					.append($('<label>')
						.text(modes[legs[i].mode.toUpperCase()] + ' ' + legs[i].route))
					.append($('<br>'))
					.append($('<br>'))
					.append($('<label>')
						.css('float', 'left')
					...
					.append($('<label>')
						.css('float', 'right')
						.text(UI.formatTime(legs[i].startTime) + (!legs[i].realTime ? '' : ' (' + UI.formatDelay(legs[i].startTime, legs[i].departureDelay) + ')')))
					.append($('<br>'))
					.append($('<label>')
						.attr('class', 'intStop')
						.html("&darr;"))
				...
				trips.last()
					...
					.append($('<label>')
						.css('float', 'right')
						.text(UI.formatTime(legs[i].endTime) + (!legs[i].realTime ? '' : ' (' + UI.formatDelay(legs[i].endTime, legs[i].arrivalDelay) + ')')))
					.append($('<br>'))
					.append($('<br>'))
